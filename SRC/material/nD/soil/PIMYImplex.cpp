...
void* OPS_PIMYImplex()
{
	...
	static int kudos = 0;
	if (++kudos == 1)
		opserr << "PIMYImplex nDmaterial - Written:  \n";
    ...
    char * arg[] = {"nd", "rho", "refShearModul", "refBulkModul",
		    ...
		    "numberOfYieldSurf (=20)", "<-implex>"};
    if (argc < (3+numParam)) {
	opserr << "WARNING insufficient arguments\n";
	opserr << "Want: nDMaterial PIMYImplex tag? " << arg[0];
	opserr << "? "<< "\n";
	...
	opserr << "WARNING invalid PIMYImplex tag" << "\n";
	return theMaterial;
    }

    int nd;
    if (OPS_GetIntInput(&numdata, &nd) < 0) {
	opserr << "WARNING invalid PIMYImplex nd" << "\n";
	return theMaterial;
    }
	...
	opserr << "WARNING invalid PIMYImplex double inputs" << "\n";
	return theMaterial;
    }
	...
	opserr << "WARNING invalid PIMYImplex numberOfYieldSurf" << "\n";
	return theMaterial;
    }
    ...
    theMaterial =
	new PIMYImplex (tag, nd, param[0], param[1], param[2],
					param[3], param[4], param[5], param[6],
					param[7], numberOfYieldSurf, gredu, implex);
...
const Vector& PIMYImplex::getImplexSateVariables(bool energy)
{
	static Vector sv(2);
	if (energy) {
...
int PIMYImplex::updateInternal(bool doExplicit, bool doTangent)
{
	int loadStage = loadStagex[matN];
	int numOfSurfaces = numOfSurfacesx[matN];
	double pressDependCoeff = pressDependCoeffx[matN];
	...
	if (loadStage != 1) {  //linear elastic
		...
		if (doImplex && doExplicit) {
			...
			Ce = 2 * refShearModulus * IIdev() + refBulkModulus * IIvol();
			...
			workV6.addVector(-1.0, currentStress.deviator(), 1.0);
			workV6 *= chi;
			workV6 *= lambda;
			doubledotProduct(mflow, workV6, Ce);
			
			// do stress relaxation
			mflow.addVector(-1.0, trialStress.t2Vector(), 1.0);
			trialStress.setData(mflow);
