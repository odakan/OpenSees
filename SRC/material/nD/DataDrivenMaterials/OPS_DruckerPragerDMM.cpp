...
OPS_DruckerPragerDMM(void)
{
	...
	static int kudos = 0;
	if (++kudos == 1)
		opserr << "Drucker-Prager Multi-Yield Surface Hardening-Softening nDmaterial - Written: OD.Akan, G.Camata, E.Spacone, CG.Lai, IUSS Pavia\n";
	...
	if (numArgs < 1)
	{
		opserr << "Want: nDMaterial DruckerPragerDMM tag? rho? Kref? Gref? Pref? modn? cohesion? TNYS? <HParams?> <-intType type?>\n";
		return theMaterial;
	}
	...
	{
		opserr << "OPS_DruckerPragerDMM: WARNING: invalid tag? (must be an integer)" << endln;
		return theMaterial;
	}
	...
	{
		opserr << "OPS_DruckerPragerDMM: WARNING: invalid rho? (must be a double)\n";
		return theMaterial;
	}
	...
	{
		opserr << "OPS_DruckerPragerDMM: WARNING: invalid Kref? Gref? Pref? modn? cohesion? (must be doubles)\n";
		return theMaterial;
	}
	...
	{
		opserr << "OPS_DruckerPragerDMM: WARNING: invalid TNYS? (must be an integer -" << maxTNYS << " <= TNYS <= " << maxTNYS << " && TNYS != 0)\n";
		return theMaterial;
	}
	...
	if (TNYS < 0)
	{
		...
		{
			opserr << "OPS_DruckerPragerDMM: WARNING: invalid HParams? (must be " << TNYS << " many doubles)\n";
			return theMaterial;
		}
	}
	//other inputs
	...
	while (OPS_GetNumRemainingInputArgs() > 0) {
		...
		if (strcmp(inputstring, "-intType") == 0) {									// #1 read type of integration 
			...
			if (OPS_GetIntInput(&numData, &integrationType) < 0) {
				opserr << "OPS_DruckerPragerDMM: WARNING: invalid integration type after -intType (must be an integer)\n";
				return theMaterial;
			}
		}
	}
