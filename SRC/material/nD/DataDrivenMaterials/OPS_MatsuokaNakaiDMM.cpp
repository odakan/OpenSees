...
// $Source: /usr/local/cvs/OpenSees/SRC/material/nD/DataDrivenMaterials/OPS_MatsuokaNakaiDMM.cpp $
// $Revision: 1.0 $
...
// Description: This file contains the implementation for the OPS_MatsuokaNakaiDMM function.
 ...
 /*----+----+----+----+----+----+----+----+----+----+----+----+----+----+----+----*
 |                                                                                |
 |                  OPS_MatsuokaNakaiDMM initializer for the                      |
...
void HelpMN(void) {
	opserr << "\n";
	opserr << "MatsuokaNakaiDMM Help: \n";
	opserr << "----------------------------------------------------------------------------------------------------------------------------------------------------------------\n";
	opserr << "nDMaterial MatsuokaNakaiDMM tag? -K Kref? -G Gref? -P Pref? <-R Rho? -M Modn? -t tnys? -c cohesion? -f frictionAngle? -d dilatancyAngle -s peakShearStrain? -implex?>\n";
...
OPS_MatsuokaNakaiDMM(void)
{
	...
	static int kudos = 0;
	if (++kudos == 1)
		opserr << "Matsuoka-Nakai Data-Driven Multi-scale nDmaterial - Written: OD.Akan, G.Camata, E.Spacone, CG.Lai, IUSS Pavia \n";
	...
	if (numArgs < 3) {
		while (OPS_GetNumRemainingInputArgs() > 0) {
			...
			if (strcmp(inputstring, "-help") == 0) {
				HelpMN();
				opserr << "FATAL: OPS_MatsuokaNakaiDMM() - Program terminated since '-help' is the first option in the MatsuokaNakaiDMM nDmaterial declaration.\n";
	...
	if (numArgs < 7) {
		opserr << "FATAL: OPS_MatsuokaNakaiDMM() - please define at least -> nDMaterial MatsuokaNakaiDMM tag? -K Kref? -G Gref? -P Pref? for linear elastic analysis...\n";
		HelpMN();
		return theMaterial;
	}
	...
	if (OPS_GetInt(&numData, &tag) != 0) {
		opserr << "FATAL: OPS_MatsuokaNakaiDMM() - invalid tag? (must be an integer)\n\n";
		HelpMN();
		return theMaterial;
	}
	...
	while (OPS_GetNumRemainingInputArgs() > 0) {
		...
		if (strcmp(inputstring, "-K") == 0) {
			...
			if (OPS_GetDoubleInput(&numData, &Kref) < 0) {
				opserr << "FATAL: OPS_MatsuokaNakaiDMM() - invalid Kref value after flag: -K (must be double)\n";
				return theMaterial;
			}
		}
		...
		if (strcmp(inputstring, "-G") == 0) {
			...
			if (OPS_GetDoubleInput(&numData, &Gref) < 0) {
				opserr << "FATAL: OPS_MatsuokaNakaiDMM() - invalid Gref value after flag: -G (must be double)\n";
				return theMaterial;
			}
		}
		...
		if (strcmp(inputstring, "-P") == 0) {
			...
			if (OPS_GetDoubleInput(&numData, &Pref) < 0) {
				opserr << "FATAL: OPS_MatsuokaNakaiDMM() - invalid Pref value after flag: -P (must be double)\n";
				return theMaterial;
			}
		}
		...
		if (strcmp(inputstring, "-R") == 0) {
			...
			if (OPS_GetDoubleInput(&numData, &rho) < 0) {
				opserr << "FATAL: OPS_MatsuokaNakaiDMM() - invalid rho value after flag: -r (must be double)\n";
				return theMaterial;
			}
		}
		...
		if (strcmp(inputstring, "-M") == 0) {
			...
			if (OPS_GetDoubleInput(&numData, &modn) < 0) {
				opserr << "FATAL: OPS_MatsuokaNakaiDMM() - invalid modn value after flag: -m (must be double)\n";
				return theMaterial;
			}
		}
		...
		if (strcmp(inputstring, "-t") == 0) {
			...
			if (OPS_GetIntInput(&numData, &TNYS) < 0) {
				opserr << "FATAL: OPS_MatsuokaNakaiDMM() - invalid TNYS value after flag: -T (must be integer)\n";
				return theMaterial;
			}
			if (abs(TNYS) > maxTNYS || TNYS == 0) {
				opserr << "FATAL: OPS_MatsuokaNakaiDMM() - invalid TNYS value! (must be an integer TNYS <= " << maxTNYS << " and TNYS cannot be 0)\n";
				return theMaterial;
			}
		...
		if (strcmp(inputstring, "-c") == 0) {
			...
			if (OPS_GetDoubleInput(&numData, &cohesion) < 0) {
				opserr << "FATAL: OPS_MatsuokaNakaiDMM() - invalid cohesion value after flag: -c (must be double)\n";
				return theMaterial;
			}
		}
		...
		if (strcmp(inputstring, "-f") == 0) {
			...
			if (OPS_GetDoubleInput(&numData, &frictionAngle) < 0) {
				opserr << "FATAL: OPS_MatsuokaNakaiDMM() - invalid frictionAngle value after flag: -f (must be double)\n";
				return theMaterial;
			}
		}
		...
		if (strcmp(inputstring, "-d") == 0) {
			...
			if (OPS_GetDoubleInput(&numData, &dilatancyAngle) < 0) {
				opserr << "FATAL: OPS_MatsuokaNakaiDMM() - invalid dilatancyAngle value after flag: -d (must be double)\n";
				return theMaterial;
			}
		}
		...
		if (strcmp(inputstring, "-s") == 0) {
			...
			if (OPS_GetDoubleInput(&numData, &peakShearStrain) < 0) {
				opserr << "FATAL: OPS_MatsuokaNakaiDMM() - invalid peakShearStrain value after flag: -s (must be double)\n";
				return theMaterial;
			}
		}
		...
		if (strcmp(inputstring, "-ddType") == 0) {
			...
			if (OPS_GetDoubleInput(&numData, &dataDriverType) < 0) {
				opserr << "FATAL: OPS_MatsuokaNakaiDMM() - invalid data driver type after flag: -ddType (must be integer)\n";
				opserr << "FATAL: OPS_MatsuokaNakaiDMM() - [$val is a double. -1: user custom surface, 0 : hyperbolic backbone, 1 : offline wheras\n";
				opserr << "FATAL: OPS_MatsuokaNakaiDMM() -         a value between 0 and 1 is the strain dicretization step for the online method]\n";
				return theMaterial;
			}
		}
		...
		if (strcmp(inputstring, "-hModuli") == 0) {
			...
			int numArgs = OPS_GetNumRemainingInputArgs();
			if (numArgs < abs(TNYS)) {
				opserr << "FATAL: OPS_MatsuokaNakaiDMM() - please enter G Modulus vector for each yield surface after flag: -hModuli (must be doubles)\n";
				return theMaterial;
			}
			// recive
			...
			if (OPS_GetDoubleInput(&numData, HModuli) < 0) {
				opserr << "FATAL: OPS_MatsuokaNakaiDMM() - invalid G Moduli! (must be " << TNYS << " many doubles  after flag: -hModuli)\n";
				return theMaterial;
			}
			hmod = Vector(TNYS);
		...
		if (strcmp(inputstring, "-hDilatancies") == 0) {
			...
			int numArgs = OPS_GetNumRemainingInputArgs();
			if (numArgs < abs(TNYS)) {
				opserr << "FATAL: OPS_MatsuokaNakaiDMM() - please enter Dilatancy vector for each yield surface after flag: -hDilatancies (must be doubles)\n";
				return theMaterial;
			}
			// recive
			numData = int(abs(TNYS));
			...
			if (OPS_GetDoubleInput(&numData, HDilatancies) < 0) {
				opserr << "FATAL: OPS_MatsuokaNakaiDMM() - invalid Dilatancies! (must be " << TNYS << " many doubles  after flag: -hDilatancies)\n";
				return theMaterial;
			}
			dps = Vector(TNYS);
		...
		if (strcmp(inputstring, "-hStrains") == 0) {
			...
			int numArgs = OPS_GetNumRemainingInputArgs();
			if (numArgs < abs(TNYS)) {
				opserr << "FATAL: OPS_MatsuokaNakaiDMM() - please enter vector of Hardening Parameters for each yield surface after flag: -hStrains (must be doubles)\n";
				return theMaterial;
			}
			// recive
			numData = int(abs(TNYS));
			...
			if (OPS_GetDoubleInput(&numData, HParams) < 0) {
				opserr << "FATAL: OPS_MatsuokaNakaiDMM() - invalid Hardening Parameters! (must be " << TNYS << " many doubles  after flag: -hStrains)\n";
				return theMaterial;
			}
			hps = Vector(TNYS);
			...
			if (totalLength > 1) {
				...
				try {
					...
					if (!file_stream.is_open()) {
						opserr << "FATAL: OPS_MatsuokaNakaiDMM() - failed to open the file at: " << fullPath << "\n";
	...
	if (Kref == 0) {
		opserr << "FATAL: OPS_MatsuokaNakaiDMM() - please enter a valid Kref value! Current Kref = " << Kref << ". Kref cannot be 0!\n";
		HelpMN();
		return theMaterial;
	}

	if (Gref == 0) {
		opserr << "FATAL: OPS_MatsuokaNakaiDMM() - please enter a valid Gref value! Current Gref = " << Gref << ". Gref cannot be 0!\n";
		HelpMN();
		return theMaterial;
	}
	...
	if (Pref < 101) {
		...
		opserr << "WARNING: OPS_MatsuokaNakaiDMM() - please double check the Pref value in use! Current Pref = " << Pref << "!\n";
	}

	if (beVerbose) {
		opserr << "\nOPS_MatsuokaNakaiDMM\n";
		opserr << "---------------------------------------------\n";
		opserr << "tag              : " << tag << "\n";
		opserr << "rho              : " << rho << "\n";
		opserr << "Kref             : " << Kref << "\n";
	...
	if (theData == nullptr) {
		...
		opserr << "FATAL: OPS_MatsuokaNakaiDMM() - yield surface data yielded bad result...";
		return theMaterial;
	}

	// create a MatsuokaNakaiDMM nDmaterial object
	theMaterial = new MatsuokaNakaiDMM(tag, rho, Kref, Gref, Pref, modn, theData, dataDriverType, integrationType, beVerbose);
